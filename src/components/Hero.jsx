import { useEffect, useRef, useState } from 'react'

const roles = ['Frontend Developer', 'UI/UX Enthusiast', 'React Developer']

export default function Hero() {
  const nameRef = useRef(null)
  const [displayName, setDisplayName] = useState('')
  const [roleIndex, setRoleIndex] = useState(0)
  const [displayRole, setDisplayRole] = useState('')
  const [isDeleting, setIsDeleting] = useState(false)
  const fullName = 'ARAVINDDAKSHAN S'

  // Typewriter for name (runs once)
  useEffect(() => {
    let i = 0
    const interval = setInterval(() => {
      setDisplayName(fullName.slice(0, i + 1))
      i++
      if (i === fullName.length) clearInterval(interval)
    }, 80)
    return () => clearInterval(interval)
  }, [])

  // Typewriter for cycling roles
  useEffect(() => {
    const current = roles[roleIndex]
    let timeout

    if (!isDeleting && displayRole.length < current.length) {
      timeout = setTimeout(() => setDisplayRole(current.slice(0, displayRole.length + 1)), 80)
    } else if (!isDeleting && displayRole.length === current.length) {
      timeout = setTimeout(() => setIsDeleting(true), 2000)
    } else if (isDeleting && displayRole.length > 0) {
      timeout = setTimeout(() => setDisplayRole(current.slice(0, displayRole.length - 1)), 40)
    } else if (isDeleting && displayRole.length === 0) {
      setIsDeleting(false)
      setRoleIndex((prev) => (prev + 1) % roles.length)
    }
    return () => clearTimeout(timeout)
  }, [displayRole, isDeleting, roleIndex])

  return (
    <section className="hero" id="hero">
      {/* Floating Orbs */}
      <div className="orb orb-1" />
      <div className="orb orb-2" />
      <div className="orb orb-3" />

      <div className="hero-content">
        {/* Profile Photo */}
        <div className="hero-photo-wrapper">
          <div className="photo-glow" />
          <div className="photo-ring" />
          <img
            src="/assets/profile.jpeg"
            alt="Aravinddakshan S"
            className="hero-photo"
          />
        </div>

        {/* Name with typewriter */}
        <h1 className="hero-name" ref={nameRef}>
          {displayName}
          {displayName.length < fullName.length && (
            <span className="cursor-blink">|</span>
          )}
        </h1>

        {/* Cycling role */}
        <div className="hero-role">
          <span className="hero-role-text">{displayRole}</span>
          <span style={{ color: 'var(--accent-blue)', animation: 'blink 0.7s step-end infinite' }}>|</span>
        </div>

        <p className="hero-subtext">
          Building beautiful, high-performance web experiences
        </p>

        {/* CTA Buttons */}
        <div className="hero-buttons">
          <a
            href="#experience"
            className="btn-primary"
            onClick={e => { e.preventDefault(); document.querySelector('#experience')?.scrollIntoView({ behavior: 'smooth' }) }}
          >
            <span>⚡ View My Work</span>
          </a>
          <a href="/assets/resume.pdf" className="btn-secondary" target="_blank" rel="noreferrer">
            <span>↓ Download Resume</span>
          </a>
        </div>

        {/* Social Icons */}
        <div className="hero-socials">
          <a href="mailto:mailtoaravindd03@gmail.com" className="social-icon" aria-label="Email" title="Email">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>
            </svg>
          </a>
          <a href="https://github.com/" className="social-icon" target="_blank" rel="noreferrer" aria-label="GitHub">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z"/>
            </svg>
          </a>
          <a href="https://linkedin.com/" className="social-icon" target="_blank" rel="noreferrer" aria-label="LinkedIn">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
              <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
            </svg>
          </a>
          <a href="tel:+919840402289" className="social-icon" aria-label="Phone" title="Phone">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.4 2 2 0 0 1 3.6 1.22h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.77a16 16 0 0 0 6.29 6.29l.94-.94a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>
            </svg>
          </a>
        </div>
      </div>

      {/* Scroll indicator */}
      <div className="scroll-indicator">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <path d="M12 5v14M5 12l7 7 7-7"/>
        </svg>
        <span>Scroll</span>
      </div>
    </section>
  )
}
